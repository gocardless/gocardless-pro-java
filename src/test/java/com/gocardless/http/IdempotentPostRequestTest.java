...
public class IdempotentPostRequestTest {
    ...
    public void shouldUseSameIdempotencyKeyForFurtherRequestsIfNotSet() throws Exception {
        ...
        http.assertRequestMade("POST", "/dummy", "fixtures/single.json", ImmutableMap.of("Authorization", "Bearer token", "Idempotency-Key", idempotencyKey, "Accept-Language", "fr-FR"));
        http.assertRequestMade("POST", "/dummy", "fixtures/single.json", ImmutableMap.of("Authorization", "Bearer token", "Idempotency-Key", idempotencyKey, "Accept-Language", "fr-FR"));
    }
