...
public class PayerThemesCreateForCreditorCodeSampleTest {
    ...
    @Test
    public void testCreateForCreditorCodeSample() throws Exception {
        // Mock response - enqueue multiple times to handle code samples with multiple API calls
        String responseBody = "{ \"payer_themes\": {} }";
        for (int i = 0; i < 5; i++) {
        ...
        try {
            PayerTheme payerTheme = client.payerThemes().createForCreditor()
                    .withButtonBackgroundColour("#128DAA").withContentBoxBorderColour("#BD10E0")
                    .withHeaderBackgroundColour("#BD10E0").withLinkTextColour("#7ED321")
