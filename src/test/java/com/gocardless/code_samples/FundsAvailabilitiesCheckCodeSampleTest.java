...
public class FundsAvailabilitiesCheckCodeSampleTest {
    ...
    @Test
    public void testCheckCodeSample() throws Exception {
        // Mock response - enqueue multiple times to handle code samples with multiple API calls
        String responseBody = "{ \"funds_availability\": {} }";
        for (int i = 0; i < 5; i++) {
        ...
        try {
            client.fundsAvailabilities().check("MD123").withAmount("1000").execute();
        } finally {
