...
public class ScenarioSimulatorsRunCodeSampleTest {
    ...
    @Test
    public void testRunCodeSample() throws Exception {
        // Mock response - enqueue multiple times to handle code samples with multiple API calls
        String responseBody = "{ \"scenario_simulators\": {} }";
        for (int i = 0; i < 5; i++) {
        ...
        try {
            client.scenarioSimulators().run("payment_failed").withLinksResource("PM123").execute();
        } finally {
