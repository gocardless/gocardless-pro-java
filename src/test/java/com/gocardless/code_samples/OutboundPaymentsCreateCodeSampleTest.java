...
public class OutboundPaymentsCreateCodeSampleTest {
    ...
    public void testCreateCodeSample() throws Exception {
        ...
        try {
            OutboundPayment outboundPayment = client.outboundPayments().create().withAmount(1000)
                    .withScheme(Scheme.FASTER_PAYMENTS)
                    .withDescription("Reward Payment (August 2024)").withReference("Invoice 123")
                    .withLinksCreditor("CR123").withLinksRecipientBankAccount("BA123").execute();
        } finally {
