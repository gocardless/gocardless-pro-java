...
public class EventsListCodeSampleTest {
    ...
    public void testListCodeSample() throws Exception {
        ...
        String responseBody = "{ \"events\": [{}], \"meta\": { \"cursors\": {}, \"limit\": 50 } }";
        for (int i = 0; i < 5; i++) {
        ...
        try {
            for (Event event : client.events().all().withResourceType(ResourceType.PAYMENTS)
                    .execute()) {
