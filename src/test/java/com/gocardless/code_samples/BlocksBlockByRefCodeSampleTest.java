...
public class BlocksBlockByRefCodeSampleTest {
    ...
    @Test
    public void testBlockByRefCodeSample() throws Exception {
        // Mock response - enqueue multiple times to handle code samples with multiple API calls
        String responseBody = "{ \"blocks\": [{}], \"meta\": { \"cursors\": {}, \"limit\": 50 } }";
        for (int i = 0; i < 5; i++) {
        ...
        try {
            for (Block block : client.blocks().blockByRef().withReferenceType("customer")
