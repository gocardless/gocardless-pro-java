...
public class ScenarioSimulator {
     ...
     * <ul>
     * <li>`creditor_verification_status_action_required`: Sets a creditor's `verification status`
     ...
     * payment for ACH, BECS, BECS_NZ or SEPA, in which cases the mandate may be
     * `pending_submission`, since their mandates are submitted with their first payment).</li>
     * <li>`payment_charged_back`: Behaves the same as the `payout_paid_out` simulator, except that
     * the payment is transitioned to `charged_back` after it is paid out, having been charged back
     * by the customer.</li>
     * <li>`payment_chargeback_settled`: Behaves the same as the `payment_charged_back` simulator,
     ...
     * <li>`mandate_activated`: Transitions a mandate through to `activated`, having been submitted
     ...
     * submitted with their first payment.</li>
     * <li>`mandate_expired`: Transitions a mandate through to `expired`, having been submitted to
     ...
     * `pending_submission` state. Only compatible with Bacs mandates.</li>
     * <li>`mandate_suspended_by_payer`: Transitions a mandate to `suspended_by_payer`, as if payer
     * has suspended the mandate after it has been setup successfully. It must start in the
     * `activated` state. Only compatible with PAY_TO mandates.</li>
     * <li>`refund_paid`: Transitions a refund to `paid`. It must start in either the
     * `pending_submission` or `submitted` state.</li>
     * <li>`refund_settled`: Transitions a refund to `paid`, if it's not already, then generates a
     ...
     * state.</li>
     * <li>`billing_request_fulfilled`: Authorises the billing request, and then fulfils it. The
     ...
     * <li>`billing_request_fulfilled_and_payment_failed`: Authorises the billing request, fulfils
     ...
     * <li>`billing_request_fulfilled_and_payment_paid_out`: Authorises the billing request, fulfils
