...
* <li>`payment_confirmed`: Transitions a payment through to `confirmed`. It must start in the
...
* apparently collected successfully beforehand. It must start in either the `pending_submission` or
* `paid_out` state, and its mandate must be in the `activated` state (unless it is a payment for
* ACH, BECS, BECS_NZ or SEPA, in which cases the mandate may be `pending_submission`, since their
* mandates are submitted with their first payment). Not compatible with Autogiro mandates.</li>
...
* payment.</li>
* <li>`mandate_customer_approval_granted`: Transitions a mandate through to `pending_submission`, as
* if the customer approved the mandate creation. It must start in the `pending_customer_approval`
* state. Compatible only with Bacs and SEPA mandates, which support customer signatures on the
* mandate. All payments associated with the mandate will be transitioned to `pending_submission`.
* All subscriptions associated with the mandate will become `active`.</li>
* <li>`mandate_customer_approval_skipped`: Transitions a mandate through to `pending_submission`, as
* if the customer skipped the mandate approval during the mandate creation process. It must start in
* the `pending_customer_approval` state. Compatible only with Bacs and SEPA mandates, which support
* customer signatures on the mandate. All payments associated with the mandate will be transitioned
...
* <li>`mandate_suspended_by_payer`: Transitions a mandate to `suspended_by_payer`, as if payer has
* suspended the mandate after it has been setup successfully. It must start in the `activated`
* state. Only compatible with PAY_TO mandates.</li>
* <li>`refund_paid`: Transitions a refund to `paid`. It must start in either the
* `pending_submission` or `submitted` state.</li>
* <li>`refund_settled`: Transitions a refund to `paid`, if it's not already, then generates a payout
* that includes the refund, thereby settling the funds. It must start in one of
...
* <li>`refund_returned`: Transitions a refund to `refund_returned`. The refund must start in
* `pending_submission`.</li>
* <li>`payout_bounced`: Transitions a payout to `bounced`. It must start in the `paid` state.</li>
* <li>`billing_request_fulfilled`: Authorises the billing request, and then fulfils it. The billing
* request must be in the `pending` state, with all actions completed except for
* `bank_authorisation`. Only billing requests with a `payment_request` are supported.</li>
...
* <li>`billing_request_fulfilled_and_payment_confirmed_to_failed`: Authorises the billing request,
* fulfils it, moves the associated payment to `confirmed` and then moves it to `failed`. The billing
* request must be in the `pending` state, with all actions completed except for
* `bank_authorisation`. Only billing requests with a `payment_request` are supported.</li>
